...
from libreadqe import (
    read_pos_and_etot_ratio,
    read_wave,
    get_ratio_folder,
    read_eig,
    get_save_folder,
...
from libmath import (
    overlap_x_quantum_harmonic,
    report_cache_gauss_miss,
    overlap_x_quantum_harmonic_num,
...
def calc_dQ(vecR, list_pos1, list_pos2):
    ...
    :return: dQ, Delta Q between two structure
            ...
            ar_Qi : normalized phono mode, (1/sqrt(M) * M_i * mu ) for all i-th directions, \mu is the phonon mode vector, in 3*N array (xyz change first, atom index second)
    """
    list_delta_pos = [
    ...
    list_delta = [
        {"species": atom["species"], "delta": pos}
        for atom, pos in zip(list_pos1, list_delta_pos)
    ]

    list_R = [norm(atom["delta"]) ** 2 for atom in list_delta]
    list_mR = [
        f_Element_Symbol_to_Mass(atom["species"]) * norm(atom["delta"]) ** 2
        for atom in list_delta
    ]
    dQ2 = sum(list_mR)
    dR2 = sum(list_R)
    dQ = sqrt(dQ2)
    ...
    ar_Qi = np.asarray(
        [
            atom["delta"]
            * f_Element_Symbol_to_Mass(atom["species"])
            / sqrt(M)
            / sqrt(dR2)
            for atom in list_delta
        ]
...
def calc_freq(folder, ratio_min, ratio_max, dQ):
    ...
    # grab ratio folders, file names, and values
    ratio_folders = [name for name in os.listdir(folder) if name.startswith("ratio-")]
    ratio_files = [os.path.join(folder, name, "scf.out") for name in ratio_folders]
    ratio_values = [float(name.strip("ratio-")) for name in ratio_folders]

    for ratio, filename in zip(ratio_values, ratio_files):
    ...
    print(
        "Found: %i points in folder %s in range %.2f ~ %.2f"
        % (len(list_data), folder, ratio_min, ratio_max)
    )
    ...
    for order in range(2, min(5, len(list_data) + 1)):
        ...
        try:
            hfreq = (
                1
                / dQ
                * sqrt(p[order - 2] * 2 * Electron2Coulomb / (Ang2m ** 2 * AMU2kg))
                * hbar_eVs
            )
...
def calc_wif_standalone(dir1, dir2, ix_defect, ix_bandmin, ix_bandmax, de=None):
    """
    Compute Wif from a series of SCF calculations; Check Phys. Rev. B 90, 075202(2014) for definition. Each SCF calculation must be in ratio-* folder and contains a *.save folder as result
    ...
    :param de: energy difference for two levels; if not specified, use energy difference at Q=0 (ratio=0) of the final stat
    """
    ...
    for iband in range(ix_bandmin, ix_bandmax + 1):
        ...
        if de is None:
            dE = (
                -ar_eig2_q0[0, spin1 - 1, iband - 1, 0]
                + ar_eig2_q0[0, spin2 - 1, ix_defect - 1, 0]
            )
        else:
            dE = de
        print("ei-ef : %.2f" % dE)
        ...
        for data in list_data1:
            ...
            if folder2 is None:
                print(
                    "Skip ratio %.4f as missing .save folder %s"
                    % (ratio, dir1 if folder1 is None else dir2)
                )
                continue
            ...
            print("%.6f %24.16g %24.16g %24.16g" % (dE, abs(s), s.real, s.imag))
...
def calc_wif(
    dir_i, dir_f, ix_defect, ix_bandmin, ix_bandmax, dQ, de=None, spinname="down"
):
    """
    Compute Wif from a series of SCF calculations; Check Phys. Rev. B 90, 075202(2014) for definition. Each SCF calculation must be in ratio-* folder and contains a *.save folder as result
    ...
    :param bandmax: Valence hole band index maximum(1-based)
    :param de: energy difference for two levels; if not specified, use energy difference at Q=0 (ratio=0) of the final stat
    :param spin: compute the overlap of wavefunction of given spin channcel, can be either "up" or "down". For a defect  all filled initial state and a hole in spin down channcel final state (PRB 90, 075202), the "down" spin should be investigate. For a defect-defect transition the selected defect level spin should be used.

    :return: dic_eig : dic(intial/final) = array of [ratio, eigenvalues for different bands], array of [ratio,  occupations numbers for different bands]
    """
        ...
        for data in list_data:
            ...
            for spin in (1, 2):
                dic_eig_all[(statename, spin)].append(
                    [
                        ratio,
                        ar_eig1[0, spin - 1, ix_plotmin - 1 : ix_plotmax, 0],
    ...
    for key, val in dic_eig_all.items():
        dic_eig_occ[key] = {
            "eig": np.asarray([[x[0]] + x[1].tolist() for x in val]),
    ...
    for iband in range(ix_bandmin, ix_bandmax + 1):
        ...
        dE = float(
            -ar_eig1_q0[0, spin - 1, iband - 1, 0]
            + ar_eig1_q0[0, spin - 1, ix_defect - 1, 0]
        )
        # Read wavefunction of a perturbed bulk state in final state (This and defect wavefunction should be same Hamiltonian to be meaningful)
        try:
        ...
        for data in list_data2:
            ...
            if folder2 is None:
                print(
                    "Skip ratio %.4f as missing .save folder %s"
                    % (ratio, os.path.join(dir_f, "ratio-%.4f" % ratio))
                )
                continue
...
def calc_phonon_part_T0_HR(dE, dQ, freq):
    ...
    for m in range(mmax, mmin, -1):
        ...
        overlap = overlap_x_quantum_harmonic_ladder_hr(m, dQ, 1, freqi)

        s1 += overlap ** 2 * coef
        print(
            "Contribution: n m occupation deltacoef overlap %3i %3i %14.7g %14.7g"
            % (n, m, coef, overlap ** 2)
        )
    s += s1
    ...
    for T in list_T:
        ...
        for i in range(1, nmax + 1):
            ...
            if n_b1 < tr:
                ...
                break
        print(
            "Temperature : %4i K ; Maximum initial state oscillator quantum number %4i"
            % (T, nmax)
        )
        ...
        for n in range(nmax, -1, -1):
            ...
            for m in range(mmax, mmin, -1):
                ...
                s1 += overlap ** 2 * coef
                print(
                    "Contribution: n m occupation deltacoef overlap %3i %3i %14.7g %14.7g %14.7g"
...
def calc_cp_T(dim, vol, g, wif, list_phonon_part):
    ...
    for temperature, osc in list_phonon_part:
        cp = (
            vol * 2 * pi * g * wif ** 2 * osc * (Bohr2m ** dim) * 1e2 ** dim * second2au
        )
