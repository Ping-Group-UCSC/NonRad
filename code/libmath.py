...
# Enable mpmath?
# If not using mpmath the summation of Gaussian integrals are not stable as there are
# lots of very large numbers and small numbers
...
# Constant = sqrt(1/(2^(m+n) * pi * m! * n!)) * La^(-m-n) * (m1\omega_1)^(n/2+1/4) \
#               * (m2\omega_2)^(m/2+1/4) * exp(-m1\omega1m2\omega2 Sb^2/La)
...
def overlap_x_gaussian(ni, nf, oi, of, Ri, mi, freqi, mf, freqf, order_x):
    ...
    Calculate <chi_fm|Q-Q0|chi_in> where Q0 is equalibrium geomeotry of chi_f,
        only one Gaussian term in Hermite equation
...
# Integral and coef
            ...
            if (b_mp):
                t1 = mpmath.power(2, -mpf(ni+nf-oi-of)/2.0) * \
                    mpmath.sqrt(mpf(1.0) * factorial(oi)/factorial(ni) * factorial(of)/factorial(nf))
            else:
...
def eval_gaussian(x, n_level, n_order, R, m, freq):
    ...
    x = x - R
    mfreq = m * freq
    y = 1 / sqrt(1.0 * factorial(n_level) * 2**n_level) * (mfreq / pi) ** 0.25 * \
        exp(-mfreq*x**2/2) * (x * sqrt(mfreq))**n_order
...
def calc_gaussian_x_alpha_val_num(n, m, Ri, momega1, momega2, order_x=1):
    ...
    y, abserr = scipy.integrate.quad(
        eval_overlap_x_gaussian, -np.inf, np.inf, args=(n, m, n, m, Ri, 1, momega1, 1, momega2, order_x),
        limit=500, epsrel=1e-14, epsabs=1e-14
    )
#   print(y, abserr)
    return y
...
def overlap_x_gaussian_num(ni, nf, oi, of, Ri, mi, freqi, mf, freqf, order_x):
    y, abserr = scipy.integrate.quad(
        eval_overlap_x_gaussian, -np.inf, np.inf, args=(ni, nf, oi, of, Ri, mi, freqi, mf, freqf, order_x),
        limit=500, epsrel=1e-14, epsabs=1e-14
    )
#   print(y, abserr)
    return y
...
def eval_quantum_harmonic(x, n, R, m, freq):
    ...
    '''
    x = x - R
    y = 1 / sqrt(factorial(n) * 2**n) * (m * freq / pi) ** 0.25 * \
        exp(-m*freq*x**2/2) * eval_hermite_polynomial(n, x * sqrt(m*freq))
    return y
