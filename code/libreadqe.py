...
def read_pos_and_etot_ratio(folder=None):
    ...
    for ratio, filename in zip(ratio_values, ratio_files):
        ...
        with open(filename.replace(".in", ".out"), "r") as f:
            ...
            if tag != "!" and tag != "!!":
                print(
                    "Cannot recognize total energy in %s"
    ...
    try:
        ...
        for ik in range(1, nk + 1):
            for ispin, stspin in enumerate(list_spin):
                filename = t1.find(
                    "EIGENVALUES/K-POINT.%i/DATAFILE%s" % (ik, stspin)
                ).attrib["iotk_link"]
                t2 = ET.parse(os.path.join(folder, filename)).getroot()
                data = np.asarray(
                    [float(x) for x in t2.find("EIGENVALUES").text.split()]
                )
                occ = np.asarray(
                    [float(x) for x in t2.find("OCCUPATIONS").text.split()]
                )
