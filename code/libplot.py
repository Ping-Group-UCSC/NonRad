...
#   np.savetxt(os.path.join(dir_image, "%s.dat" % casename), data)
    ...
    for i in range(ar_eig.shape[1]-1):
        ...
        ax.plot(ar_dQ, ar_eig[:,i+1], marker="o", linestyle="-", color="black", fillstyle = fillstyle, markerfacecolor=markerfacecolor)
...
#Plot points
        ar_Q = ar0[:,0] * dQ
        ax.plot(ar_Q, ar0[:,1], "bo", marker="o", color=list_color[ix], fillstyle = "full", markerfacecolor="white",label="Band %i" % iband)
